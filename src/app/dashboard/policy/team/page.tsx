...
export default function OurTeamManagerPage() {
  ...
  useEffect(() => {
    fetch(`${BACKEND}/api/admin/pages/our-team`, {
  ...
  async function handleSave() {
    ...
    setSaveMsg(null)
    try {
      const res = await fetch(`${BACKEND}/api/admin/pages/our-team`, {
        method: 'POST',
        headers: {
      ...
      setSaveMsg({ text: 'Our Team page saved successfully.', ok: true })
    } catch (err) {
  ...
  return (
    <div className="dash-page">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 16 }}>
        <div>
          <h2 style={{ fontSize: 20, fontWeight: 600, color: '#1a1a2e', margin: 0 }}>Our Team Manager</h2>
          <p style={{ color: 'rgba(0,0,0,0.4)', fontSize: 14, marginTop: 4 }}>
            Edit the content displayed on <strong>/our-team</strong>.
